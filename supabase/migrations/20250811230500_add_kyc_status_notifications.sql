-- Create trigger and function to insert a notification when a user's KYC status changes
-- This uses the kyc_documents table as the source of truth

-- Safety: wrap in a transaction
begin;

-- Function: notify user on KYC status change
...
-- Trigger: after status updates on kyc_documents
create trigger trg_notify_kyc_status_change
after update of status on public.kyc_documents
for each row
execute procedure public.notify_kyc_status_change();
