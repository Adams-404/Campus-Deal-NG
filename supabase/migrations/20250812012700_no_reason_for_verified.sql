-- Do not append admin notes for 'verified' status in notify_kyc_status_change()
...
begin
  if tg_op = 'UPDATE' and (old.status is distinct from new.status) and new.user_id is not null then
    ...
    -- Only include admin notes for non-verified statuses
    if v_status <> 'verified' and v_reason is not null then
      v_content := v_content || ' Reason: ' || v_reason;
    end if;

    -- Find/update-or-insert, with uuid/text compare fixed
    ...
    if found then
      ...
      where n2.user_id = new.user_id
        and n2.type = v_type
        and (n2.metadata ->> 'document_id') = new.id::text
